...
app.get('/exportar-excel', async (req, res) => {
  try {
    ...
    db.query(query, async (err, results, fields) => {
      ...
      // Cabeçalhos dinâmicos com ajuste especial para a coluna 'data'
      const colunas = fields.map(field => ({
        header: field.name.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase()),
        key: field.name,
        width: field.name === 'data' ? 12 : 20  // Reduz largura da coluna 'data'
      }));
      ...
      // Autoajuste de largura com exceção de 'data'
      ...
      // Congela a primeira linha e primeira coluna
      sheet.views = [
