...
app.post("/upload", upload.single("arquivo"), (req, res) => {
  ...
  fs.createReadStream(caminho)
    .pipe(csv({ separator: ";" }))
    .on("data", (row) => {
      const colunaA = Object.values(row)[0];
      const cnpj = colunaA.match(/\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}/)?.[0] || null;

      if (cnpj) {
        registros.push({ razao_social: colunaA, cnpj });
      }
    })
    .on("end", () => {
      if (!registros.length) {
        fs.unlinkSync(caminho);
        return res.send("Nenhum dado válido encontrado.");
      }

      // Buscar os CNPJs que já existem
      const cnjpsUnicos = [...new Set(registros.map(r => r.cnpj))];

      db.query(
        `SELECT cnpj FROM razao_social WHERE cnpj IN (?)`,
        [cnjpsUnicos],
        (err, rows) => {
          if (err) {
            console.error("Erro ao consultar CNPJs existentes:", err);
            fs.unlinkSync(caminho);
            return res.status(500).send("Erro ao verificar dados.");
          }

          const existentes = new Set(rows.map(row => row.cnpj));
          const novos = registros.filter(r => !existentes.has(r.cnpj));

          if (novos.length === 0) {
            fs.unlinkSync(caminho);
            return res.send("Nenhum novo registro para salvar.");
          }

          const dadosParaInserir = novos.map(r => [r.razao_social, r.cnpj]);
          ...
          db.query(
            ...
            (err) => {
              ...
              if (err) {
                console.error("Erro ao salvar novos registros:", err);
                return res.status(500).send("Erro ao salvar dados.");
              }

              return res.send(`✅ ${novos.length} novos registros foram adicionados.`);
            }
          );
        }
      );
