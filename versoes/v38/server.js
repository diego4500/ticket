...
app.get('/tickets', (req, res) => {
  ...
  SELECT 
      ...
      status,
      DATE_FORMAT(data, '%d/%m/%Y') AS data_abertura,
      chamado,
      descricao
  FROM tickets
...
app.post('/atualizar-descricao', (req, res) => {
  ...
  const sql = "UPDATE tickets SET descricao = ?, status = ? WHERE ticket = ?";
  db.query(sql, [descricao, status, ticket], (err, result) => {
    if (err) {
...
app.get('/tickets-filtrado', (req, res) => {
  const status = req.query.status; // 'aberto', 'fechado', ou undefined (todos)
  ...
  t.status,
  DATE_FORMAT(t.data, '%d/%m/%Y') AS data_abertura,

  t.chamado,
  t.descricao,
  CASE rs.cliente WHEN 1 THEN 'Sim' ELSE 'Não' END AS cliente
FROM tickets t
LEFT JOIN razao_social rs ON rs.razao_social = t.razao_social
  `;
  ...
  sql += ` ORDER BY ticket DESC LIMIT ? OFFSET ?`;
  ...
  try {
    ...
    db.query(query, async (err, results, fields) => {
      ...
      const colunas = fields
        .filter(field => field.name !== 'algum_campo_indesejado') // se quiser remover algo
        .map(field => ({
...
app.get('/exportar-excel-funcionalidade', async (req, res) => {
  try {
    ...
    db.query(query, async (err, results, fields) => {
      ...
      // Remover as colunas que você não quer exportar
      const colunasParaExcluir = ['titulo', 'menu_duvida', 'churn', 'sistema'];

      // Filtra os campos do banco, removendo os indesejados
      ...
      // Remover as colunas indesejadas dos dados também
...
app.get('/exportar-excel-duvidas', async (req, res) => {
  try {
    ...
    db.query(query, async (err, results, fields) => {
      ...
      // ❌ Colunas que você quer excluir
      const colunasParaExcluir = ['churn', 'funcionalidade', 'sistema'];
      ...
      // ✅ Filtra os dados para remover as colunas excluídas
      const dadosFiltrados = results.map(row => {
        const novo = {};
        camposFiltrados.forEach(coluna => {
          novo[coluna.key] = row[coluna.key];
...
app.get('/exportar-excel-churn', async (req, res) => {
  try {
    ...
    db.query(query, async (err, results, fields) => {
      ...
      const colunasParaExcluir = ['titulo', 'menu_duvida', 'funcionalidade', 'sistema'];

      // ✅ Só usa as colunas que devem aparecer
      const camposFiltrados = fields
        .filter(field => !colunasParaExcluir.includes(field.name))
        .map(field => ({
          header: field.name.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase()),
      ...
      // ✅ Adiciona só os dados das colunas desejadas
      const dadosFiltrados = results.map(row => {
        const novo = {};
        camposFiltrados.forEach(coluna => {
          novo[coluna.key] = row[coluna.key];
