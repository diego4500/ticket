...
app.get('/tickets-filtrado', (req, res) => {
  ...
  if (status === 'abertos') {
    sql += ` WHERE t.status = 'Aberto'`;
  } else if (status === 'fechados') {
    sql += ` WHERE t.status = 'Fechado'`;
  }
    ...
    db.query(query, async (err, results, fields) => {
      ...
      const colunas = fields
        ...
        .map(field => ({
          ...
          key: field.name,
          width: field.name === 'data_abertura' ? 12 : 20
        }));
      ...
      sheet.columns.forEach(column => {
        if (column.key !== 'data') {
          ...
          if (column.key === 'data_abertura') {
            column.width = 12; // largura ideal para datas: DD/MM/AAAA
          } else {
            column.width = Math.min(maxLength, 30); // evita colunas muito largas
          }
        }
      });
