...
app.get('/tickets-filtrado', (req, res) => {
  const status = req.query.status; // 'aberto', 'fechado', ou undefined
  ...
  let sql = `
    SELECT 
      ...
      t.chamado,
      t.descricao,
      CASE TRIM(rs.cliente) WHEN '1' THEN 'Sim' ELSE 'Não' END AS cliente
  ...
  // ✅ Adiciona filtro de status, se houver
  if (status) {
    sql += ` WHERE t.status = ?`;
    params.push(status.toLowerCase()); // garante minúsculo
  }
